...
# az vm run-command で実行する場合
az vm run-command invoke `
    --resource-group rg-onprem-nested `
    --name vm-onprem-nested-hv01 `
    --command-id RunPowerShellScript `
    --scripts @scripts/Setup-NestedNetwork.ps1
```
...
# az vm run-command で実行する場合（非同期・タイムアウト 1 時間）
az vm run-command create `
    --resource-group rg-onprem-nested `
    --vm-name vm-onprem-nested-hv01 `
    --name CreateNestedVMs `
    --script @scripts/Create-NestedVMs.ps1 `
...
# (3) 固定 IP 設定
.\scripts\Configure-StaticIPs.ps1
...
│   ├── Configure-StaticIPs.ps1  # ゲスト VM 固定 IP 設定 (ホスト VM)
│   ├── Install-ADDS.ps1         # AD DS インストール・DC 昇格 (ホスト VM)
│   ├── Join-Domain.ps1          # ドメイン参加 (ホスト VM)
│   ├── Setup-HybridDns.ps1     # ハイブリッド DNS 構成 (ホスト VM)
