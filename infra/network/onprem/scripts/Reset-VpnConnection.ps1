...
$cnOnprem = az network vpn-connection show -g $OnpremResourceGroup -n cn-onprem-to-hub --query '{name:name, status:connectionStatus}' -o json 2>$null | ConvertFrom-Json
$cnHub    = az network vpn-connection show -g $HubResourceGroup -n cn-hub-to-onprem --query '{name:name, status:connectionStatus}' -o json 2>$null | ConvertFrom-Json
$lgwHub   = az network local-gateway show -g $OnpremResourceGroup -n lgw-hub --query 'name' -o tsv 2>$null
$lgwOnprem = az network local-gateway show -g $HubResourceGroup -n lgw-onprem --query 'name' -o tsv 2>$null

$resources = @()
if ($cnOnprem)  { $resources += "cn-onprem-to-hub  ($OnpremResourceGroup) [$($cnOnprem.status)]" }
if ($cnHub)     { $resources += "cn-hub-to-onprem  ($HubResourceGroup) [$($cnHub.status)]" }
if ($lgwHub)    { $resources += "lgw-hub           ($OnpremResourceGroup)" }
if ($lgwOnprem) { $resources += "lgw-onprem        ($HubResourceGroup)" }
...
if ($cnOnprem -or $cnHub) {
    ...
    if ($cnOnprem) { az network vpn-connection wait -g $OnpremResourceGroup -n cn-onprem-to-hub --deleted 2>$null }
    if ($cnHub)    { az network vpn-connection wait -g $HubResourceGroup -n cn-hub-to-onprem --deleted 2>$null }
