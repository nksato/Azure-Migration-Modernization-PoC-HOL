...
`infra/network/onprem/main.bicep` で、疑似オンプレ (`vnet-onprem`) と Hub (`vnet-hub`) の間に Site-to-Site VPN 接続を確立します。
...
| 設定方法 | `infra/network/onprem/Setup-HybridDns.ps1` または手動 |
...
| `infra/network/onprem/main.bicep` | VPN Gateway 配置・S2S 接続・ピアリング Gateway Transit |
