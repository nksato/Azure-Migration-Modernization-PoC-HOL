...
スクリプトは以下の 3 ステップを実行します（[3/3] はオプション）。

| ステップ | 方向 | 内容 |
|---|---|---|
| [1/3] | オンプレ → クラウド | DC01 に `privatelink.database.windows.net` → DNS Resolver Inbound IP の条件付きフォワーダーを追加 |
| [2/3] | クラウド → オンプレ | DNS Forwarding Ruleset（`dnsrs-hub`）を作成し、`lab.local` → DC01（`10.0.1.4`）の転送ルールを追加 |
| [3/3] | オンプレ → クラウド VM | Private DNS Zone（`azure.internal`）を作成し、Spoke VM の A レコードを自動登録。DC01 に条件付きフォワーダーを追加（`-EnableCloudVmResolution` 指定時のみ） |

> **オプションスイッチ:**
>
> | スイッチ | 用途 | 使うタイミング |
> |---|---|---|
> | `-LinkSpokeVnets` | Forwarding Ruleset を Spoke1〜4 VNet にもリンク | Spoke VM から `lab.local` を名前解決したいとき |
> | `-EnableCloudVmResolution` | [3/3] を実行し、オンプレからクラウド VM をホスト名で解決可能にする | 移行シナリオで必要になった場合 |
>
> ```powershell
> # 基本実行（[1/3] + [2/3] のみ）
> .\infra\network\Setup-HybridDns.ps1
...
# (オプション) ルールセットを Spoke VNet にもリンク
# Spoke VM からオンプレの名前解決が必要な場合のみ実行
# スクリプト使用時は -LinkSpokeVnets スイッチを指定:
#   .\Setup-HybridDns.ps1 -LinkSpokeVnets
foreach ($i in 1..4) {
  $rg = "rg-spoke$i"; $vnet = "vnet-spoke$i"
