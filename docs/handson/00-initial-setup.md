...
- **英大文字 / 英小文字 / 数字 / 記号** を組み合わせて、推測されにくい値にします
- クラウド側と疑似オンプレ側で **完全に同じ値** を使用してください
- 日本語、空白、引用符（`"` / `'`）、バックスラッシュ（`\`）など、入力やコピー時に扱いにくい文字は避けると安全です

例:

```text
PocHol-VPN-2026!EastJp#SecureKey42
```

> **注意**: 上記は形式の一例です。**そのまま使わず**、各環境ごとに十分にランダムな別の値へ置き換えてください。
...
>
> 推奨は **32 文字以上** のランダムな文字列です。英大文字 / 英小文字 / 数字 / 記号を組み合わせ、日本語や空白、引用符を避けると扱いやすくなります。
