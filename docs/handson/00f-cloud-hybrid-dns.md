...
> **Note**  
> このページは [`00-initial-setup.md`](./00-initial-setup.md) の **方法 A「デプロイ後に必要な手動設定」** および **方法 B — Step 4** の詳細手順です。  
> 方法 A で Deploy to Azure を使った場合も、DC01 への DNS 条件付きフォワーダー設定は自動化されないため、このページの手順を実行してください。
...
> テンプレートの実体は [`infra/cloud/modules/network/dns-forwarding-ruleset.bicep`](../../infra/cloud/modules/network/dns-forwarding-ruleset.bicep) です。
...
> スクリプトで一括実行する場合は [`infra/onprem/scripts/Setup-DnsForwarder.ps1`](../../infra/onprem/scripts/Setup-DnsForwarder.ps1) を利用できます。
>
> ```powershell
> .\infra\onprem\scripts\Setup-DnsForwarder.ps1
> ```
