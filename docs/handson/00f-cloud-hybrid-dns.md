...
VPN 接続（[00e](./00e-cloud-vpn-connect.md)）を確立しただけでは、疑似オンプレ側とクラウド側の間で**名前解決ができません**。

- 疑似オンプレ側から `privatelink.database.windows.net`（Azure SQL の Private Endpoint）を解決できない
- クラウド側から `lab.local`（Active Directory ドメイン）を解決できない

このハンズオンでは、**Azure DNS Private Resolver** を中心にした双方向の DNS 転送で、VPN 越しの名前解決を実現します。
